...
function cliInterface({
  ...
  targetAppConfigPath = targetAppConfigPath || nodeCommandArgument.targetConfig || envrironmentArgument.targetConfig || ownConfiguration.externalApp.configurationFilePath // default configuration path
  /**
  ...
  console.assert(scriptKeyToInvoke, '\x1b[41m%s\x1b[0m', '❌ scriptKeyToInvoke must be set.')
  ...
  ;({ path: targetAppConfigPath } = configurationFileLookup({
    ...
    currentDirectory,
    configurationBasePath: ownConfiguration.externalApp.configurationBasePath
  }))
